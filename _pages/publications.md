---
layout: archive
title: "Publications"
permalink: /publications/
author_profile: true
---
## **2025**
- **Xinyang Tian**, Xiaolong Yu, Jianwei Niu, Wei Wang. Passivity-Based Impedance Control With Energy-Aware and Safety Null Space Compliance for 7-DOF Redundant Robots. ***IEEE Transactions on Industrial Electronics***, vol. 73, no.3, pp. 4354-4364, 2025.
- **Xinyang Tian**. Tracking Control of 7-DOF Redundant Manipulators with Enhanced Null Space Compliance. ***2025 IEEE/RSJ International Conference on Intelligent Robots and Systems (IROS 2025)***, 2025, pp. 18551-18557.
- **Xinyang Tian**, Xiaolong Yu, Qinhuan Xu, Jianwei Niu, Wei Wang. Robot impedance control with restricted energy and power under contact loss, ***Control and Decision(控制与决策)***. (EI)

## **2024**

- **Xinyang Tian**, Ziyang Cheng, Mingde Bi, Jiefan Zhu. Ultra-Compact Joint Torque Sensor Units with Enhanced Resolution for Collaborative Robots. ***IEEE Sensors Journal***, vol. 24, no. 8, pp. 12074-12084, 2024.
- Lingxiao Jiang, **Xinyang Tian**, Qiang Zhan, et al. Impedance Control of an Anthropomorphic Hands Without Finger Force Sensors. ***IEEE Transactions on Automation Science and Engineering***. vol. 21, no. 4, pp. 5779-5789.


## **2023**

- **Xinyang Tian**, Qiang Zhan, and Yin Zhang. Simplified Configuration Design of an Anthropomorphic Hand Imitating Specific Human Hand Grasps. ***IEEE Robotics and Automation Letters***, vol.8, no.1, pp. 152-159, 2022.
- **Xinyang Tian**, Qiang Zhan, and Qinhuan Xu. Flexible Joint Controller Design based on desired impedance behavior, ***Journal of Harbin Engineering University（哈尔滨工程大学学报）***. (EI)


## **2021**

- **Xinyang Tian**, Qiang Zhan. A Hermaphrodite Electromechanical Connector for Self-reconfigurable Robot Modules, ***IEEE/ASME Transactions on Mechatronics***, vol. 26, no. 6, pp. 3276-3281, 2021. 
- **Xinyang Tian**, Qinhuan Xu, and Qiang Zhan. An analytical inverse kinematics solution with joint limits avoidance of 7-DOF anthropomorphic manipulators without offset, ***Journal of the Franklin Institute***, vol.358, no.2, pp. 1252-1272, 2021. 

## **2019**

- **Xinyang Tian**, Qiang Zhan. Design of ArmBot: A Self-Reconfigurable Robot Module with Electromechanical Connectors, In Proc. *2019 4th International Conference on Robotics and Automation Engineering (ICRAE)*, 2019, pp. 163-167.

# Reviewer
- ICRA 2025
- ICRA 2024 IROS 2024
- IEEE Robotics and Automation Letters 
